...
CREATE TABLE t_book(
	...
	`book_book_name` VARCHAR(100),
	`book_book_price` DECIMAL(11,2),
	`book_book_author` VARCHAR(100),
	`book_book_sales` INT,
	`book_book_stock` INT,
	`book_book_img_path` VARCHAR(200)

);

## 插入初始化测试数据
INSERT INTO t_book(`id` , `book_name` , `book_author` , `book_price` , `book_sales` , `book_stock` , `book_img_path`)
...
## 查看表内容
SELECT id, book_name, book_author, book_price, book_sales, book_stock, book_img_path FROM t_book;
